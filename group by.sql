--GROUP BY --
/*
Group by cümleciği bir select ifadesinde satırları sutunları degerlerine göre
ozet olarak gruplamak için kullanılır
Group By cumleciği her grup basına satır döndürür
Group by  genelde ; AVG(), COUNT(), MIN(), SUM() gibi aggregate foksiyonları ile birlikte kullanılır
*/
...
--satılan meyve türüne (urun_adi) göre satılan (urun_miktari) MIN ve MAX urun miktrlarını,
--MAX urun miktarına göre  sıralayarak listeleyen sorguyu yazınız
...
-- kisi ismine ve urun adına gore satılan urunlerin topamını gruplandıran
--ve isme göre ters sırasında listeleyen sorguyu yazınız
select isim, urun_adi, sum(urun_miktari) as toplam_urun from manav
group by isim, urun_adi
order by isim desc;
